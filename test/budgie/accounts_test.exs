defmodule Budgie.AccountsTest do
  ...
  describe "name_changeset/2" do
    ...
    test "validates minimum length of 2 characters" do
      user = %User{}
      attrs = %{name: "J"}
    ...
    test "validates maximum length of 100 characters" do
      user = %User{}
  ...
  describe "update_name/2" do
    ...
    test "successfully updates name with valid input", %{user: user} do
      assert {:ok, updated_user} = Accounts.update_user_name(user, %{name: "New Name"})
      assert updated_user.name == "New Name"
    end

    test "returns error when name is missing", %{user: user} do
      assert {:error, changeset} = Accounts.update_user_name(user, %{})
      assert "can't be blank" in errors_on(changeset).name
    end

    test "returns error when name is too short", %{user: user} do
      assert {:error, changeset} = Accounts.update_user_name(user, %{name: "A"})
      assert "should be at least 2 character(s)" in errors_on(changeset).name
    end

    test "returns error when name is too long", %{user: user} do
      assert {:error, changeset} =
               Accounts.update_user_name(user, %{name: String.duplicate("a", 101)})
